...
# Save scalers
with open(os.path.join(suppl_path, f'{modality_name}_scaler_features_fold_{algorithms[algorithm]}_{folds[fold]}.pkl'), 'wb') as f:
    pickle.dump(scaler_features, f)
    
with open(os.path.join(suppl_path, f'{modality_name}_scaler_target_fold_{algorithms[algorithm]}_{folds[fold]}.pkl'), 'wb') as f:
...
# Save model
with open(os.path.join(models_path, f'{modality_name}_stacked_{algorithms[algorithm]}_{merge_type}_model_fold_{folds[fold]}.pkl'), 'wb') as f:
    pickle.dump(model, f)
...
pd.DataFrame({'eid': target_real_train_id, 'g_pred_stack_train': target_pred_train}).to_csv(
    os.path.join(g_pred_path, f'{modality_name}_target_pred_2nd_level_{algorithms[algorithm]}_{merge_type}_train_fold_{folds[fold]}.csv'),
    index=False
)

pd.DataFrame({'eid': target_real_test_id, 'g_pred_stack_test': target_pred_test}).to_csv(
    os.path.join(g_pred_path, f'{modality_name}_target_pred_2nd_level_{algorithms[algorithm]}_{merge_type}_test_fold_{folds[fold]}.csv'),
    index=False
)
...
# Write results
results_file = os.path.join(models_path, f'{modality_name}_{algorithms[algorithm]}_{merge_type}_stacked_result_fold_{folds[fold]}.csv')
